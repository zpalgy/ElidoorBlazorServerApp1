...
namespace BlazorServerApp1.Data
{
	public static class PrApiCalls
	{
		...
		public static List<CYLGRP4DECOR_Class> lstCylGrp4Decor = new List<CYLGRP4DECOR_Class>();
		...
		public static void InitInMemoryDataSources(ref string errMsg)
		{
			try
			{
				...
				lstCylGrp4Decor = getCylGrps4Decor(ref errMsg);
		...
		public static List<TRSH_LOCKHINGE_DRILH_Class> getLockHingeDrilHeights(ref string errMsg)
		{
			try
			{
				...
				if (response.IsSuccessful)
				{
					...
					emptyLockHingeDrilH.LOCKDRILHEIGHT = 0;
					emptyLockHingeDrilH.MEASURENAME = "ללא";
		...
		public static List<TRSH_LOCKHINGE_DRILH_Class> getLockHingeDrilHeights1(DoorConfig doorConfig)
		{
			...
			foreach (TRSH_LOCKHINGE_DRILH_Class LockDH in lstLock_Hinge_Dril_Heights)
			{
				if (LockDH.LOCKDRILHEIGHT > 0 && LockDH.LOCKDRILHEIGHT < doorConfig.DOORHEIGHT && LockDH.TRSH_DOOR_HWCATCODE == doorConfig.TRSH_DOOR_HWCATCODE)
				{
					lstLockDH1.Add(LockDH);
				}
			}
		...
		public static List<TRSH_HARDWARE_Class> getHardwares(ref string errMsg)
		{
			try
			{
				...
				string fields = "TRSH_HARDWARE,TRSH_DOOR_HWCATCODE,PARTNAME,PARTDES,OPENSIDE,OPENSIDE_RIGHT,OPENSIDE_LEFT,OPENSIDE_RIGHTLEFT,"
								+ "DRIL4HW,DRIL4HWDES,COLORED,NIKEL,BRONZE,FORHALFCYL,IS4DECOR,TRSH_HWGRP4CYLS,PARTNAME2,OPPOSITESIDE_PART";
		...
		public static List<CYLGRP4DECOR_Class> getCylGrps4Decor(ref string errMsg)
		{
			try
			{
				RestClient restClient = new RestClient();
				initRestClient(restClient);
				RestRequest request = new RestRequest();
				string fields = "CYLGRP4DECOR_LINE,TRSH_CYLGRP4DECOR,TRSH_HWGRP4CYLS,HWGRP4CYLSDES,TRSH_CYLINDER,PARTNAME,PARTDES,DECOROUT,DECORIN,DECOR_OUTIN,OPENOUT,OPENIN";
				request.Resource = string.Format("TRSH_CYLGRP4DECOR?$select={0}", fields);
				IRestResponse response = restClient.Execute(request);
				if (response.IsSuccessful)
				{
					var settings = new JsonSerializerSettings
					...
					ValuesCYLGRP4DECOR_Class val = JsonConvert.DeserializeObject<ValuesCYLGRP4DECOR_Class>(response.Content);
					List<CYLGRP4DECOR_Class> val1 = new List<CYLGRP4DECOR_Class>();  //val.value;

					foreach (CYLGRP4DECOR_Class cyl in val.value)
					{
						val1.Add(cyl);
					}
					return val1;
...
@"""EXTSEPLINESCLRID"":0,""EXTCPLATEHTDMNDCLRID"":0,""EXTPERIFPROFILECLRID"":0,""EXTCGRIDPLATECLRID"":0," +

@"""EXTCGRIDBKGDCLRID"":0,""EXTHTPLATESCLRID"":0,""ATTACHEDPLATECLRID"":0,""EXTGLASSPLATECLRID"":0,""EXTMODERNPLATECLRID"":0,""DECORPLATECLRID"":0" +
...
@",""EXTFINMODERNSEPLINE"":null,""EXTFINMODERNPLATE"":null,""INTCOLORID"":0,""INTSIDE_C_PLATE"":null,""INTGRIDCPLATEDES"":null,""INTHTPLATE"":null" +
@",""CENTRALCOLCLRID"":0,""SHALVANIACLRID"":0,""INTPERIFPROFILECLRID"":0,""INTSIDECNTRPLATECLID"":0,""INTSEPLINESCLRID"":0" +
@",""INTCGRIDPLATECLRID"":0,""INTCGRIDCLRID"":0,""INTHTPLATESCLRID"":0,""INTGLASSPLATECLRID"":0,""INTMODERNPLATECLRID"":0,""INTNIROSTALINESCLRID"":0" +
